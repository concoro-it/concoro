  ...
  ({ className }: { className?: string }) => {
    ...
    if (shouldReduceMotion) {
      return (
        <div
          className={cn(
            "absolute h-full w-full inset-0 flex items-center justify-center opacity-30",
            className,
          )}
        >
          <svg
        ...
        >
          ...
          {paths.map((path, index) => (
            <motion.path
              ...
              strokeOpacity={isMobile ? "0.3" : "0.4"}
              strokeWidth="0.5"
            />
          ))}
          ...
          <defs>
            {paths.map((path, index) => (
              <motion.linearGradient
                ...
                transition={{
                  duration: isMobile 
                    ? Math.random() * 8 + 12  // Slower on mobile (12-20s)
                    : Math.random() * 10 + 10, // Original speed (10-20s)
                  ease: "easeInOut",
                  repeat: Infinity,
                  delay: isMobile 
                    ? Math.random() * 15  // Longer delays on mobile
                    : Math.random() * 10, // Original delays
                }}
              >
