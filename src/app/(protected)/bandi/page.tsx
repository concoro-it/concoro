...
function JobsPage() {
  ...
  const toDate = (dateValue: unknown): Date | null => {
    ...
    if (typeof dateValue === 'object' && 'seconds' in dateValue) {
      const timestamp = dateValue as { seconds: number };
  ...
  const updateURL = useCallback((page: number, jobId?: string) => {
  ...
  }, [jobs, selectedJob, initialUrlHandled, currentPage, searchParams, updateURL]) // Run when jobs load and no job is selected
  ...
  }, [filteredJobs, isMobile, selectedJob, currentPage, updateURL]) // Only run when jobs change or no job selected
