...
// Function to update jobs in Firestore
async function updateJobStatus(data: any[]) {
  try {
    ...
    for (const job of data) {
      ...
      const stato = isDatePassed(job.data_scadenza) ? 'Chiuso' : 'Aperto';
      
      // Use set with merge instead of update to handle both new and existing documents
      batch.set(docRef, {
        Stato: stato,
...
async function main() {
  try {
    ...
    const updatedCount = await updateJobStatus(jobData);
