...
// ----------------------
// Firestore Retrieval (Server-side)
// ----------------------
  ...
  try {
    ...
    for (const id of concorsoIds) {
      try {
        ...
        if (docSnap.exists) {
          const data = docSnap.data() as JobDetails;
        ...
        console.error(`Error fetching concorso with ID ${id}:`, error);
      }
    }
...
// ----------------------
// User Profile Retrieval (Server-side)
// ----------------------

export async function getUserProfile(userId: string): Promise<UserData | undefined> {
  try {
    const db = initializeFirebaseAdmin();
    ...
    if (userSnap.exists) {
      ...
      if (data) {
        return mapUserProfileToUserData(data);
      }
    }
    return undefined;
  } catch (error) {
...
// This function is used to convert from Firestore Timestamp to regular Date
...
export function mapUserProfileToUserData(profile: UserProfile): UserData {
  return {
    ...
    experience: profile.experience?.map(exp => ({
      ...
      startDate: convertTimestamp(exp.startDate),
      endDate: convertTimestamp(exp.endDate),
    })),
    education: profile.education?.map(edu => ({
      school: edu.schoolName,
      degree: edu.degree,
      field: edu.fieldOfStudy,
      startDate: convertTimestamp(edu.startDate),
      endDate: convertTimestamp(edu.endDate),
    })),
  };
} 