...
class BrevoEmailService {
  ...
  constructor() {
    this.apiKey = functions.config().brevo?.api_key || process.env.BREVO_API_KEY || '';
    if (!this.apiKey) {
      functions.logger.warn('BREVO_API_KEY is not configured - emails will not be sent');
    }
  }

  async sendTransactionalEmail(emailData: TransactionalEmailData): Promise<any> {
    if (!this.apiKey) {
  ...
  private generateNotificationEmailHTML(userName: string, notifications: any[]): string {
    const urgentNotifications = notifications.filter(n => n.daysLeft === 0);
    const soonNotifications = notifications.filter(n => n.daysLeft === 1);
    const upcomingNotifications = notifications.filter(n => n.daysLeft > 1);
    ...
    const generateNotificationCard = (notification: any, bgColor: string, urgencyText: string) => `
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #f4f4f4;">
  ...
  private generateNotificationEmailText(userName: string, notifications: any[]): string {
    const formatDate = (timestamp: any) => {
      try {
        const date = timestamp?.toDate ? timestamp.toDate() : new Date(timestamp);
        return date.toLocaleDateString('it-IT');
...
export const onUserProfileUpdate = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    if (hasRequiredFields) {
      try {
        const response = await fetch('https://n8n.srv865706.hstgr.cloud/webhook/trigger-user-profile-update', {
          method: 'POST',
...
export const createScheduledNotifications = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      const userSavedConcorsos = new Map<string, Array<{docId: string, concorsoId: string, savedAt: any}>>();
...
async function sendUserEmailNotifications(db: admin.firestore.Firestore, userId: string): Promise<boolean> {
  try {
...
export const onConcorsoSaved = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      const concorsoData = concorsoDoc.data()!;
      const notificationsCreated = await createNotificationsForConcorso(
        db, 
        userId, 
        concorso_id, 
        concorsoData, 
...
async function processUserNotifications(
  ...
  userId: string, 
  savedConcorsos: Array<{docId: string, concorsoId: string, savedAt: any}>
): Promise<number> {
  ...
  for (const saved of savedConcorsos) {
    try {
      ...
      const concorsoData = concorsoDoc.data()!;
      const created = await createNotificationsForConcorso(
        db, 
        userId, 
        saved.concorsoId, 
        concorsoData, 
...
async function createNotificationsForConcorso(
  db: admin.firestore.Firestore,
  userId: string,
  concorsoId: string,
  concorsoData: any,
